...
(let* 
  ...
  (install-capability (marmalade.ledger.MINT "{TOKEN_ID}" "k:{key_pair.get_pub_key()}" 100.0))
  (marmalade.ledger.mint "{TOKEN_ID}" "k:{key_pair.get_pub_key()}" (read-keyset "nft-staker-admin") 100.0)
)
"""
    ...
    "clist": [
      {
        ...
        "args": [f"{TOKEN_ID}", f"k:{key_pair.get_pub_key()}", 100.0]
      },
      {
        "name": "coin.GAS",
        "args": []
